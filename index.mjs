...
const storage = new CloudinaryStorage({
  ...
  params: {
    folder: 'DBMS_project',
    allowerdFormat: ["png", "jpg", "jpeg", "pdf"],
  },
});
...
db.query(`CREATE TABLE IF NOT EXISTS page_8_upload (
  id INT AUTO_INCREMENT PRIMARY KEY,
  email VARCHAR(255) NOT NULL,
  ...
  bestpaper_path VARCHAR(255) NULL,
...
app.post("/reset-password/:token", (req, res) => {
  ...
  if (password !== confirm_password) {
    res.render("formpages/reset-password.ejs", {
      token,
  ...
  bcrypt.hash(password, 10, (err, hash) => {
    ...
    db.query(
      ...
      [hash, req.session.forgotpasswordemail],
      (err, result) => {
        if (err) {
...
app.get("/formpages/1", isAuthenticated, (req, res) => {
  ...
  db.query(
    "DELETE FROM applicationdetails WHERE email = ?",
    [userEmail],
    (err) => {
      if (err) {
      ...
      db.query(
        "DELETE FROM personaldetails WHERE email = ?",
        [userEmail],
        (err) => {
          if (err) {
...
app.post(
  "/formpages/1",
  isAuthenticated,
  upload.fields([{ name: "uploadid" }, { name: "userfile" }]),
  (req, res) => {
    ...
    db.query(
      "INSERT INTO applicationdetails SET ?",
      applicationdetails,
      (err, result) => {
        if (err) {
        ...
        db.query(
          "INSERT INTO personaldetails SET ?",
          personaldetails,
          (err, result) => {
            if (err) {
            ...
            res.redirect("/formpages/2"); // Redirect after database operations
          }
        );
      }
    );
...
app.get("/formpages/2", isAuthenticated, (req, res) => {
  const userEmail = req.session.currUser.email;
  let educationaldetails = req.session.educationaldetails || {};
  let edu_additionaldetails = req.session.edu_additionaldetails || {};
  // Delete existing data from all tables where email matches
  db.query(
    "DELETE FROM edu_additionaldetails WHERE educationaldetails_id IN (SELECT id FROM educationaldetails WHERE email = ?)",
    [userEmail],
    (err) => {
      if (err) {
        console.error("Error deleting publications:", err);
      ...
      db.query(
        "DELETE FROM educationaldetails WHERE email = ?",
        [userEmail],
        (err) => {
          if (err) {
            console.error("Error deleting top10publications:", err);
          ...
          db.query(
            ...
            (err, rows) => {
              ...
              res.render("formpages/2nd.ejs", {
                ...
                edu_additionaldetails: edu_additionaldetails
              });
            }
          );
        }
      ...
      if (error) {
        ...
        if (req.body.add_degree !== undefined) {
          for (let i = 0; i < req.body.add_degree.length; i++) {
            ...
            req.session.edu_additionaldetails.push(edu_additionalDetailsValues); 
            db.query(
              additionalDetailsQuery,
              edu_additionalDetailsValues,
              (error, results, fields) => {
                if (error) {
...
app.get("/formpages/3", isAuthenticated, (req, res) => {
  ...
  // Delete existing data from all tables where email matches
  db.query(
    "DELETE FROM presentemployment WHERE email = ?",
    [userEmail],
    (err) => {
      if (err) {
        return res.status(500).send("Internal Server Error");
      ...
      db.query(
        "DELETE FROM employmenthistory WHERE email = ?",
        [userEmail],
        (err) => {
          if (err) {
            return res.status(500).send("Internal Server Error");
          ...
          db.query(
            "DELETE FROM teachingexp WHERE email = ?",
            [userEmail],
            (err) => {
              if (err) {
                return res.status(500).send("Internal Server Error");
              ...
              db.query(
                "DELETE FROM researchexp WHERE email = ?",
                [userEmail],
                (err) => {
                  if (err) {
                    return res.status(500).send("Internal Server Error");
                  ...
                  db.query(
                    "DELETE FROM industrialexp WHERE email = ?",
                    [userEmail],
                    (err) => {
                      if (err) {
                        return res.status(500).send("Internal Server Error");
                      ...
                      db.query(
                        "DELETE FROM aos_aor WHERE email = ?",
                        [userEmail],
                        (err) => {
                          if (err) {
                            return res
                          ...
                          db.query(
                            ...
                            (err, rows) => {
                              ...
                              res.render("formpages/3rd.ejs", {
                                firstname: first_name,
                                lastname: last_name,
                                presentemployment: req.session.presentemployment || {},
                                aos_aor: req.session.aos_aor || {}
                              });
                            }
                          );
                        }
...
app.post("/formpages/3", isAuthenticated, (req, res) => {
  ...
  req.session.presentemployment = present || {};
  req.session.aos_aor = aos_aor || {};
  ...
  if (req.body.exp_position !== undefined) {
    for (let i = 0; i < req.body.exp_position.length; i++) {
      let data2 = [
        req.body.exp_position[i],
        req.body.exp_employer[i],
        req.body.exp_doj[i],
        req.body.exp_dol[i],
        req.body.exp_duration[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO employmenthistory(exp_position,exp_employer,exp_doj,exp_dol,exp_duration,email) Values (?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.t_exp_position !== undefined) {
    for (let i = 0; i < req.body.t_exp_position.length; i++) {
      let data2 = [
        req.body.t_exp_position[i],
        req.body.t_exp_employer[i],
        req.body.t_exp_course[i],
        req.body.t_ugpg[i],
        req.body.t_noofstudents[i],
        req.body.t_doj[i],
        req.body.t_dol[i],
        req.body.t_duration[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO teachingexp(t_exp_position,t_exp_employer,t_exp_course,t_ugpg,t_noofstudents,t_doj,t_dol,t_duration,email) Values (?,?,?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.r_exp_position !== undefined) {
    for (let i = 0; i < req.body.r_exp_position.length; i++) {
      let data2 = [
        req.body.r_exp_position[i],
        req.body.r_exp_institute[i],
        req.body.r_exp_supervisor[i],
        req.body.r_exp_doj[i],
        req.body.r_exp_dol[i],
        req.body.r_exp_duration[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO researchexp(r_exp_position,r_exp_institute,r_exp_supervisor,r_exp_doj,r_exp_dol,r_exp_duration,email) Values (?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.ind_exp_doj && Array.isArray(req.body.ind_exp_doj)) {

    for (let i = 0; i < req.body.ind_exp_doj.length; i++) {

      let data2 = [
        req.body.ind_exp_organization[i],
        req.body.ind_exp_workprofile[i],
        req.body.ind_exp_doj[i],
        req.body.ind_exp_dol[i],
        req.body.ind_exp_duration[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO industrialexp(ind_exp_organization,ind_exp_workprofile,ind_exp_doj,ind_exp_dol,ind_exp_duration,email) Values (?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
...
app.get("/formpages/4", isAuthenticated, (req, res) => {
  const userEmail = req.session.currUser.email;
  const one = req.session.one || {};
  const google_link = req.session.google_link || {};
  // Delete existing data from all tables where email matches
  db.query("DELETE FROM publications WHERE email = ?", [userEmail], (err) => {
    ...
    db.query(
      "DELETE FROM top10publications WHERE email = ?",
      [userEmail],
      (err) => {
        if (err) {
          console.error("Error deleting top10publications:", err);
        ...
        db.query("DELETE FROM patents WHERE email = ?", [userEmail], (err) => {
          ...
          db.query("DELETE FROM books WHERE email = ?", [userEmail], (err) => {
            ...
            db.query(
              "DELETE FROM book_chapters WHERE email = ?",
              [userEmail],
              (err) => {
                if (err) {
                ...
                db.query(
                  "DELETE FROM googlelink WHERE email = ?",
                  [userEmail],
                  (err) => {
                    if (err) {
                      console.error("Error deleting book_chapters:", err);
...
app.post("/formpages/4", isAuthenticated, (req, res) => {
  ...
  if (req.body.author !== undefined) {
    for (let i = 0; i < req.body.author.length; i++) {
      ...
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO top10publications(author,title,journal,year,impact,doi,status,email) Values (?,?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.pauthor !== undefined) {
    for (let i = 0; i < req.body.pauthor.length; i++) {
      let data2 = [
        req.body.pauthor[i],
        req.body.ptitle[i],
        req.body.p_country[i],
        req.body.p_number[i],
        req.body.pyear_filed[i],
        req.body.pyear_published[i],
        req.body.pyear_issued[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO patents(pauthor,ptitle,p_country,p_number,pyear_filed,pyear_published,pyear_issued,email) Values (?,?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.bauthor !== undefined) {
    for (let i = 0; i < req.body.bauthor.length; i++) {
      ...
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO books(bauthor,btitle,byear,bisbn,email) Values (?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.bc_author !== undefined) {
    for (let i = 0; i < req.body.bc_author.length; i++) {
      ...
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO book_chapters(bc_author,bc_title,bc_year,bc_isbn,email) Values (?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  req.session.google_link = req.body.google_link;
  db.query(
    "INSERT INTO googlelink(email,googlelink) Values (?,?)",
    [req.session.currUser.email, req.body.google_link],
    (err, result) => {
      if (err) throw err;
    }
  );
  ...
  db.query("DELETE FROM membership WHERE email = ?", [userEmail], (err) => {
    ...
    db.query("DELETE FROM training WHERE email = ?", [userEmail], (err) => {
      ...
      db.query("DELETE FROM awards WHERE email = ?", [userEmail], (err) => {
        ...
        db.query(
          "DELETE FROM sponsoredprojects WHERE email = ?",
          [userEmail],
          (err) => {
            if (err) {
              console.error("Error deleting books:", err);
            ...
            db.query(
              "DELETE FROM consultancyprojects WHERE email = ?",
              [userEmail],
              (err) => {
                if (err) {
                  console.error("Error deleting book_chapters:", err);
...
app.post("/formpages/5", isAuthenticated, (req, res) => {
  if (req.body.professional_society_name !== undefined) {
    for (let i = 0; i < req.body.professional_society_name.length; i++) {
      let data2 = [
        req.body.professional_society_name[i],
        req.body.membership_status[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO membership(professional_society_name, membership_status,email) Values (?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.training_type !== undefined) {
    for (let i = 0; i < req.body.training_type.length; i++) {
      let data2 = [
        req.body.training_type[i],
        req.body.training_organization[i],
        req.body.training_year[i],
        req.body.training_duration[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO training(training_type, training_organization, training_year,training_duration,email) Values (?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.award_name !== undefined) {
    for (let i = 0; i < req.body.award_name.length; i++) {
      let data2 = [
        req.body.award_name[i],
        req.body.awarded_by[i],
        req.body.award_year[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO awards(award_name, awarded_by, award_year,email) Values (?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.sponsoring_agency !== undefined) {
    for (let i = 0; i < req.body.sponsoring_agency.length; i++) {
      let data2 = [
        req.body.sponsoring_agency[i],
        req.body.project_title[i],
        req.body.sanctioned_amount[i],
        req.body.project_period[i],
        req.body.project_role[i],
        req.body.project_status[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO sponsoredprojects(sponsoring_agency, project_title, sanctioned_amount, project_period, project_role, project_status,email) Values (?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.consultancy_organization !== undefined) {
    for (let i = 0; i < req.body.consultancy_organization.length; i++) {
      let data2 = [
        req.body.consultancy_organization[i],
        req.body.consultancy_title[i],
        req.body.grant_amount[i],
        req.body.consultancy_period[i],
        req.body.consultancy_role[i],
        req.body.consultancy_status[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO consultancyprojects(consultancy_organization, consultancy_title, grant_amount,consultancy_period,consultancy_role,consultancy_status,email) Values (?,?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
...
app.post("/formpages/6", isAuthenticated, (req, res) => {
  if (req.body.phd_name !== undefined) {
    for (let i = 0; i < req.body.phd_name.length; i++) {
      let data2 = [
        req.body.phd_name[i],
        req.body.phd_title[i],
        req.body.phd_role[i],
        req.body.phd_status[i],
        req.body.phd_year[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO phd_thesis(phd_name, phd_title,phd_role,phd_status,phd_year,email) Values (?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.pg_name !== undefined) {
    for (let i = 0; i < req.body.pg_name.length; i++) {
      let data2 = [
        ...
        req.body.pg_title[i],
        req.body.pg_role[i],
        req.body.pg_status[i],
        req.body.pg_year[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO pg_thesis(pg_name, pg_title,pg_role,pg_status,pg_year,email) Values (?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
  ...
  if (req.body.ug_name !== undefined) {
    for (let i = 0; i < req.body.ug_name.length; i++) {
      let data2 = [
        ...
        req.body.ug_title[i],
        req.body.ug_role[i],
        req.body.ug_status[i],
        req.body.ug_year[i],
      ];
      data2.push(req.session.currUser.email);
      db.query(
        "INSERT INTO ug_thesis(ug_name, ug_title,ug_role,ug_status,ug_year,email) Values (?,?,?,?,?,?)",
        data2,
        (err, result) => {
          if (err) throw err;
        }
...
app.get("/formpages/7", isAuthenticated, (req, res) => {
  const userEmail = req.session.currUser.email;
  const page_7 = req.session.page_7 || {};
  db.query("DELETE FROM page_7 WHERE email = ?", [userEmail], (err) => {
    if (err) {
      console.error("Error deleting publications:", err);
      return res.status(500).send("Internal Server Error");
    }
...
app.post("/formpages/7", isAuthenticated, (req, res) => {
  ...
  research_statement = research_statement.substring(
    3,
    research_statement.length - 6
  );
  teaching_statement = teaching_statement.substring(
    3,
    teaching_statement.length - 6
  );
  rel_in = rel_in.substring(3, rel_in.length - 6);
  prof_serv = prof_serv.substring(3, prof_serv.length - 6);
  jour_details = jour_details.substring(3, jour_details.length - 6);
  conf_details = conf_details.substring(3, conf_details.length - 6);
  data.push(
    research_statement,
    teaching_statement,
    rel_in,
...
app.post(
  ...
  (req, res) => {
    let page_8_upload = {
      ...
      phd_path: req.files["phdCertificate"]
        ? req.files["phdCertificate"][0].path
        : null,
      pg_path: req.files["pgDocuments"]
        ? req.files["pgDocuments"][0].path
        : null,
      ug_path: req.files["ugDocuments"]
        ? req.files["ugDocuments"][0].path
        : null,
      tw_path: req.files["twelfthCertificate"]
        ? req.files["twelfthCertificate"][0].path
        : null,
      te_path: req.files["tenthCertificate"]
        ? req.files["tenthCertificate"][0].path
        : null,
      pay_path: req.files["paySlip"] ? req.files["paySlip"][0].path : null,
      noc_path: req.files["nocUndertaking"]
        ? req.files["nocUndertaking"][0].path
        : null,
      post_path: req.files["postPhdExperience"]
        ? req.files["postPhdExperience"][0].path
        : null,
      misc_path: req.files["miscCertificate"]
        ? req.files["miscCertificate"][0].path
        : null,
      sign_path: req.files["signature"] ? req.files["signature"][0].path : null,
      bestpaper_path: req.files["bestpaper"]
        ? req.files["bestpaper"][0].path
        : null,

    };
    req.session.signpath = page_8_upload.sign_path;
    db.query("DELETE FROM page_8_upload", (err, result) => {
      db.query(
        "INSERT INTO page_8_upload SET ?",
        page_8_upload,
        (err, result) => {
          if (err) {
